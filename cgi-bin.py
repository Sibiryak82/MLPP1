...
import cgi  # cqi.test() выведет поля ввода
import os
import shelve
import sys
...
sys.path.insert(0, os.getcwd())    # благодаря этому модуль pickle
                                   ...
                                   # в главный шаблон разметки html
replyhtml = """
<html>
...
rowhtml = '<tr><th>%s<td><input type=text name=%s value="%%(%s)s">/n'
rowshtml = ''
...
if action == 'Fetch':
    ...
    fields = dict.fromkeys(fieldnames, '?')      # недопустимые значение
